Dashboard Company Attendance Overview view
Vue 3 composition api, Typescript, pinia(should interact through pinia, don't use props), 
use short delay to simulate data api fetch, design a good component for me

Container 1 (Dashboard Company Attendance Overview view)
  >left side
    >use chart.js donut to display data of this format
    >data to show in chart has these category (Workday, NonWorkday, OnLeave, Tardiness, Absent, ShiftNotStarted)
    >default select first category
    >has a user filter dropdown(will affect employee details table in container 2) (can filter based on user list) default is all
    >has a date filter(date selected here will affect Container 2) default is today
    
  >right side employee details table(use primevue datatable, userId as data key)
    >has column of (employeeUserId, employeeId, employeeName, dayTypeId, deviceChannel, clockTime, lastClockedAt, shiftStartTime, leaves, tardiness )
    >show employee details from the category selected
 container 1 will call this first api and return this data,
   {
    "isToday": true,
    "totalCount": 106,
    "overview": [
        ...
        },
        {
            ...
            "details": [
                ...
                },
                {
                    ...
                    "leaves": [
                        {
                            "leaveType": "Compassionate Leave 123",
...
Container 2 (Clock in/out)
  >up
    >show the selected date from Container 1, call second api with filter parameter
    >display the category and its number, the category include (clockType and count) 
    >category is clickable, default select clockType 0
  >down
    >primevue datatable with employee details, userId as datakey, column include(employeeUserId, employeeId, employeeName, deviceChannel, description, clockTime)
    
 container 2 will call second api and will return this data,
{
    "clockSummary": [
        {
            "clockType": 0,
...
both container can shared to same pinia store to track the selected date
